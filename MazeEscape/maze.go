...
Line17, Line18, Line19, Line20, Line21, Line22, Line23, Line24, Line25, Line26 *svg.Line
*/
...
func InitMap() {
		...
		for i:=0; i<4; i++ {
		line := SvgMap.AddNewChild(svg.KiT_Line, fmt.Sprintf("line%v", i)).(*svg.Line)

line.SetProp("stroke", "black")

switch i {
  case 0:
  line.Start = gi.Vec2D{-7,8}
line.End = gi.Vec2D{-1,8}
case 1:
line.Start = gi.Vec2D{-1,8}
line.End = gi.Vec2D{-1,-8}
case 2:
line.Start = gi.Vec2D{1,9}
line.End = gi.Vec2D{1,-8}
case 3:
line.Start = gi.Vec2D{-7,10}
line.End = gi.Vec2D{0, 10}
}
...
func InitPlayer() {
	...
	player.SetProp("stroke", "black")
	player.Size = gi.Vec2D{1.5, 1.5}
	player.Pos = gi.Vec2D{-8, 8}
